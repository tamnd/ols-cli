...
describes what users get. Pick a format with `-o`, or let ols choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ols <command> -o table   # aligned columns for reading
ols <command> -o jsonl   # one JSON object per line, for piping
ols <command> -o json    # a single JSON array
ols <command> -o csv     # spreadsheet friendly
ols <command> -o tsv     # tab-separated
ols <command> -o url     # just the URL column
ols <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ols <command> --fields id,title,url
```
...
```bash
ols <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ols <command>            # a table, because this is a terminal
ols <command> | wc -l    # JSONL, because this is a pipe
```
